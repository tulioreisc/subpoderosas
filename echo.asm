...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15a1:	a1 d4 19 00 00       	mov    0x19d4,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    15f9:	a3 d4 19 00 00       	mov    %eax,0x19d4
}
    15fe:	5b                   	pop    %ebx
    15ff:	5e                   	pop    %esi
    1600:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    161d:	a3 d4 19 00 00       	mov    %eax,0x19d4
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    163c:	8b 15 d4 19 00 00    	mov    0x19d4,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1691:	39 05 d4 19 00 00    	cmp    %eax,0x19d4
  ...
  return freep;
    16bb:	8b 15 d4 19 00 00    	mov    0x19d4,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16dc:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
    16df:	89 15 d4 19 00 00    	mov    %edx,0x19d4
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    16f6:	c7 05 d4 19 00 00 d8 	movl   $0x19d8,0x19d4
    16fd:	19 00 00 
    1700:	c7 05 d8 19 00 00 d8 	movl   $0x19d8,0x19d8
    1707:	19 00 00 
    base.s.size = 0;
    170a:	b8 d8 19 00 00       	mov    $0x19d8,%eax
    170f:	c7 05 dc 19 00 00 00 	movl   $0x0,0x19dc
    1716:	00 00 00 
