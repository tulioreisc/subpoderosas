...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1591:	a1 d0 19 00 00       	mov    0x19d0,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    15e9:	a3 d0 19 00 00       	mov    %eax,0x19d0
}
    15ee:	5b                   	pop    %ebx
    15ef:	5e                   	pop    %esi
    15f0:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    160d:	a3 d0 19 00 00       	mov    %eax,0x19d0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    162c:	8b 15 d0 19 00 00    	mov    0x19d0,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1681:	39 05 d0 19 00 00    	cmp    %eax,0x19d0
  ...
  return freep;
    16ab:	8b 15 d0 19 00 00    	mov    0x19d0,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16cc:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
    16cf:	89 15 d0 19 00 00    	mov    %edx,0x19d0
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    16e6:	c7 05 d0 19 00 00 d4 	movl   $0x19d4,0x19d0
    16ed:	19 00 00 
    16f0:	c7 05 d4 19 00 00 d4 	movl   $0x19d4,0x19d4
    16f7:	19 00 00 
    base.s.size = 0;
    16fa:	b8 d4 19 00 00       	mov    $0x19d4,%eax
    16ff:	c7 05 d8 19 00 00 00 	movl   $0x0,0x19d8
    1706:	00 00 00 
