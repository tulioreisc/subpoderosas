...
main(int argc, char *argv[])
{
  ...
  for(i = 1; i < argc; i++){
    ...
    1053:	68 44 17 00 00       	push   $0x1744
    1058:	6a 02                	push   $0x2
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    13af:	f7 f6                	div    %esi
    13b1:	0f b6 92 64 17 00 00 	movzbl 0x1764(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    1528:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
    152b:	b8 5d 17 00 00       	mov    $0x175d,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15b1:	a1 08 2a 00 00       	mov    0x2a08,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    1609:	a3 08 2a 00 00       	mov    %eax,0x2a08
}
    160e:	5b                   	pop    %ebx
    160f:	5e                   	pop    %esi
    1610:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    162d:	a3 08 2a 00 00       	mov    %eax,0x2a08
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    164c:	8b 15 08 2a 00 00    	mov    0x2a08,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    16a1:	39 05 08 2a 00 00    	cmp    %eax,0x2a08
  ...
  return freep;
    16cb:	8b 15 08 2a 00 00    	mov    0x2a08,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16ec:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
    16ef:	89 15 08 2a 00 00    	mov    %edx,0x2a08
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    1706:	c7 05 08 2a 00 00 0c 	movl   $0x2a0c,0x2a08
    170d:	2a 00 00 
    1710:	c7 05 0c 2a 00 00 0c 	movl   $0x2a0c,0x2a0c
    1717:	2a 00 00 
    base.s.size = 0;
    171a:	b8 0c 2a 00 00       	mov    $0x2a0c,%eax
    171f:	c7 05 10 2a 00 00 00 	movl   $0x0,0x2a10
    1726:	00 00 00 
