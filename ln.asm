...
main(int argc, char *argv[])
{
  ...
  if(argc != 3){
    ...
    101a:	68 10 17 00 00       	push   $0x1710
    101f:	6a 02                	push   $0x2
    1021:	e8 ca 03 00 00       	call   13f0 <printf>
    exit();
  ...
  if(link(argv[1], argv[2]) < 0)
    ...
    104a:	68 23 17 00 00       	push   $0x1723
    104f:	6a 02                	push   $0x2
    1051:	e8 9a 03 00 00       	call   13f0 <printf>
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    138f:	f7 f6                	div    %esi
    1391:	0f b6 92 40 17 00 00 	movzbl 0x1740(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    1508:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
    150b:	b8 37 17 00 00       	mov    $0x1737,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1591:	a1 dc 29 00 00       	mov    0x29dc,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    15e9:	a3 dc 29 00 00       	mov    %eax,0x29dc
}
    15ee:	5b                   	pop    %ebx
    15ef:	5e                   	pop    %esi
    15f0:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    160d:	a3 dc 29 00 00       	mov    %eax,0x29dc
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    162c:	8b 15 dc 29 00 00    	mov    0x29dc,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1681:	39 05 dc 29 00 00    	cmp    %eax,0x29dc
  ...
  return freep;
    16ab:	8b 15 dc 29 00 00    	mov    0x29dc,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16cc:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
    16cf:	89 15 dc 29 00 00    	mov    %edx,0x29dc
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    16e6:	c7 05 dc 29 00 00 e0 	movl   $0x29e0,0x29dc
    16ed:	29 00 00 
    16f0:	c7 05 e0 29 00 00 e0 	movl   $0x29e0,0x29e0
    16f7:	29 00 00 
    base.s.size = 0;
    16fa:	b8 e0 29 00 00       	mov    $0x29e0,%eax
    16ff:	c7 05 e4 29 00 00 00 	movl   $0x0,0x29e4
    1706:	00 00 00 
